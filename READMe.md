...
### Только запросы к LLM без решения задач

Этот режим нужен, когда корпус задач уже сгенерирован и решён всеми
baseline-методами, а нужно только проверить, какую эвристику LLM выберет по
профилю задачи. Solver не запускается, solution JSON не создаётся.
...
Результат сохраняется в `llm_responses.csv`:

- `task_id,seed,task_source`
- `valid`
- `selected_heuristic`
- `llm_latency_ms`
- `llm_used_fallback`
...
Также обновляется `experiment_config.yaml` с `mode: llm_only` и
...
Опционально можно включить детальный JSON-экспорт решений для визуализации
диаграммы Ганта:

```bash
./build-gcc/FactoryPlanningTools.exe \
  --tasks=5 \
  --difficulty=small_easy \
  --export-solution-json \
  --solution-json-dir=solution_json
```

По умолчанию этот экспорт выключен, чтобы массовые эксперименты не создавали
много тяжёлых файлов. CSV остаются основным форматом для анализа группы задач,
а JSON создаётся по одному файлу на пару `задача + метод`, например
`solution_json/task_000000_directive.json`.

JSON имеет формат `SOLUTION_VISUALIZATION_V1` и содержит:
- `tools` — расписание доступности исполнителей и их загрузку;
- `works` — сроки, штрафы и список операций заказа;
- `operations` — старт/конец операции, зависимости и допустимые исполнители;
- `assignments` — фактические интервалы выполнения операций на исполнителях,
  то есть основа для диаграммы Ганта.

Для просмотра такого файла есть desktop-утилита на Python/Tkinter:

```bash
python scripts/visualize_solution.py build-gcc/solution_json/task_000000_directive.json
```
